...
export default function SalesPlanSheet() {
  ...
  const getCellSpan = (
    record: SalesPlanRowDataType,
    rowIndex: number,
    column: Column<SalesPlanRowDataType>
  ): CellSpan<SalesPlanRowDataType> | null => {
    if (column.key === "label") {
    ...
    () => [
      ...
      },
      {
        ...
        render: (value: string, record: SalesPlanRowDataType) => {
          if (record.isSpanned) return null;
          if (!value)
            return <div className="text-center text-xs text-gray-500">-</div>;
