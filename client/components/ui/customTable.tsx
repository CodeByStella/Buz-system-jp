...
export function ExcelForm({
  ...
  const getCategory = (key: string) => {
    ...
    if (key.startsWith("nonop") || key.startsWith("extraordinary"))
      return "other";
  ...
  return (
    <Card className="h-full min-h-0 flex flex-col">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className={dense ? "flex space-x-1" : "flex space-x-2"}>
            <Button
              ...
              variant="outline"
              className={dense ? "h-7 px-2 text-xs" : undefined}
            >
              計算実行
            </Button>
            <Button
      ...
      <CardContent
        className={
          dense
            ? "py-3 flex-1 min-h-0 overflow-hidden"
      ...
      >
        <div
          className={
            dense
              ? "mb-2 flex flex-wrap gap-2 text-[11px]"
              : "mb-4 flex flex-wrap gap-3 text-sm"
          }
        >
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-slate-100 border border-slate-200"></span>
            サマリ
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-yellow-50 border border-yellow-200"></span>
            人件費
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-blue-50 border border-blue-200"></span>
            販管費
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-gray-50 border border-gray-200"></span>
            製造原価
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-orange-50 border border-orange-200"></span>
            経費
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-purple-50 border border-purple-200"></span>
            営業外/特別
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="inline-block w-3 h-3 bg-yellow-100 border border-yellow-200"></span>
            チェック
          </span>
        </div>
          ...
          <Table className="pb-4">
            <TableHeader className="sticky top-0 z-10 bg-white">
              <TableRow className={dense ? "h-8" : undefined}>
                <TableHead className={dense ? "w-1/2 text-xs py-1" : "w-1/3"}>
                  項目
                </TableHead>
                <TableHead className={dense ? "w-1/2 text-xs py-1" : "w-1/3"}>
                  値
            ...
            <TableBody>
              {Object.entries(data).map(([key, cell]) => (
                <TableRow
                  key={key}
                  className={`${dense ? "h-8" : ""} ${getRowBgClass(key)}`}
                >
                  <TableCell
                    className={
                      dense ? "font-medium text-xs py-1" : "font-medium"
                    }
                  >
                    {cell.label}
                    {cell.calculated && (
                      <span
                        className={
                          dense
                            ? "ml-1 text-[10px] bg-blue-100 text-blue-800 px-1 py-0.5 rounded"
                            : "ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded"
                        }
                      >
                        計算値
                      </span>
                  ...
                  <TableCell className={dense ? "py-1" : undefined}>
                    {cell.editable ? (
                      <Input
                        ...
                        value={cell.value}
                        onChange={(e) =>
                          handleCellChange(key, parseFloat(e.target.value) || 0)
                        }
