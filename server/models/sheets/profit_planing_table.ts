...
const FinancialMetricsSchema = new Schema(
  {
    grossProfitMargin: NumberInputSchema, // 粗利率 (%) (Gross Profit Margin %)
    sales: NumberInputSchema, // 売上 (Sales)
    variableCosts: NumberInputSchema, // 変動費 (Variable Costs)
    grossProfit: NumberInputSchema, // 粗利 (Gross Profit)
    fixedCosts: NumberInputSchema, // 固定費 (Fixed Costs)
    operatingIncome: NumberInputSchema, // 営業利益 (Operating Income)
    profitMargin: NumberInputSchema, // 益率 (Profit Margin %)
  },
  { _id: false }
);
...
const ProfitPlaningTableSheetSchema = new Schema(
  {
    ...
    currentStatus: {
      grossProfitMargin: NumberInputSchema, // 粗利率 (%) (Gross Profit Margin %)
      sales: NumberInputSchema, // 売上 (Sales) - 450,000,000 from image
      variableCosts: NumberInputSchema, // 変動費 (Variable Costs) - 273,600,000 from image
      grossProfit: NumberInputSchema, // 粗利 (Gross Profit) - 176,400,000 from image
      fixedCosts: NumberInputSchema, // 固定費 (Fixed Costs) - 130,400,000 from image
      operatingIncome: NumberInputSchema, // 営業利益 (Operating Income) - 46,000,000 from image
      profitMargin: NumberInputSchema, // 益率 (Profit Margin %) - 10.2% from image
    },
...
export type ProfitPlaningTableSheetDocument = InferSchemaType<
  typeof ProfitPlaningTableSheetSchema
> & {
