...
const MainRowSchema = new Schema({
  incomeStatement: NumberInputSchema,
  manufacturingCostReport: NumberInputSchema,
});
...
const StartSheetSchema = new Schema(
  {
    ...
    cost_of_sales_total: NumberInputSchema, //売上原価 合計 (Total Cost of Sales)
    general_admin_expenses_total: NumberInputSchema, //一般管理費 合計 (Total General and Administrative Expenses)
    operating_profit: NumberInputSchema, //営業利益 (Operating Profit)
    profit_before_tax: NumberInputSchema, //税引前利益 (Profit Before Tax)
  },
  { timestamps: true }
);
